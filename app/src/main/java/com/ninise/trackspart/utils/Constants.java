package com.ninise.trackspart.utils;

public class Constants {

    public static final String SETS = "sets";
    public static final String SECONDS = "seconds";
    public static final String REST = "rest";
    public static final String SETTINGS_PREFERENCES = "settings_pref";
    public static final String SETTINGS_SPINNER = "settings_spinner";
    public static final String SETTINGS_SAVE_LAST = "settings_save_last";
    public static final String SETTINGS_SETS = "settings_sets";
    public static final String SETTINGS_SECONDS = "settings_seconds";
    public static final String SETTINGS_REST = "settings_rest";
}
